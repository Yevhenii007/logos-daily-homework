	...
	function orderProduct(count) {
		if (count < 0) {
			...
			if (module.checkBeer() < count) {
				alert(
					`Sorry, but there are ${module.checkBeer()} beer bottles left in the warehouse`
				);
			} else {
			...
			if (module.checkVine() < count) {
				alert(
					`Sorry, but there are ${module.checkVine()} vine bottles left in the warehouse`
				);
			} else {
			...
			if (module.checkPepsi() < count) {
				alert(
					`Sorry, but there are ${module.checkPepsi()} pepsi bottles left in the warehouse`
				);
			} else {
...
buy.addEventListener("click", function () {
	...
	if (firstElem(".buying__ordered").innerHTML == "") {
		...
		firstElem(
			".information"
		).innerHTML += `Всього: ${module.checkOverallPrice()} гривень`;
	}
